...
- `application_payments` table: **has NO `user_id` column** — to get user_id, JOIN with `applications` via `application_id`. Also has NO `service_fee_amount`.
...
### Question Seeding
Review platform admin features include seeding questions and case studies. `POST /api/questions/seed` inserts 25+ NCLEX-style standalone questions. `POST /api/questions/seed-case-studies` inserts 3 full case study sets (6 questions each = 18 questions total) linked to clinical scenarios via `case_study_id`.
