 ...
 * - File storage
 * 
 * Designed to be called from the orchestrator (index.ts)
 */
...
export async function convertImageToPdf(
  ...
  try {
    ...
    // Skip very large images (>6MB) to prevent CPU timeout
    // Note: Without Sharp, we can't resize, so large images will cause timeouts
    if (originalSize > 6 * 1024 * 1024) {
      ...
      console.warn(`[WORKER] [IMAGE CONVERSION] 💡 Recommendation: Resize image to <6MB before upload`)
      return null
    }
  ...
  try {
    ...
    for (const [key, docInfo] of documents.entries()) {
      ...
      if (docInfo.fileType === 'pdf') {
        ...
        checkTimeout()
        if (!convertedPdf) {
          throw new Error(`Failed to convert image to PDF: ${docInfo.name}`)
        }
