...
interface StripePaymentFormProps {
  ...
  processingPayment?: boolean // External processing state (from parent)
}
...
export function StripePaymentForm({
  ...
  // Combined loading state - show loading if either internal or external processing
  ...
  // Store file as ArrayBuffer to prevent JSON serialization issues
  ...
  // Handle file selection - read as ArrayBuffer immediately to prevent serialization
  async function handleFileSelect(file: File) {
    ...
    if (file.size > maxSize) {
      setUploadError('File is too large. Maximum size is 10MB. Please choose a smaller file.')
      return
    }

    try {
      console.log('📁 Reading file as ArrayBuffer to prevent JSON serialization')
      
      // Read file as ArrayBuffer immediately (can't be serialized to JSON)
      const arrayBuffer = await file.arrayBuffer()
      
      // Detect proper MIME type from extension
      ...
      // Store as ArrayBuffer with metadata
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      if (paymentMethod === 'mobile_banking') {
        // For mobile banking, require proof of payment data
        ...
        // Validate proof of payment data
        if (!proofOfPaymentData.arrayBuffer || proofOfPaymentData.fileSize === 0) {
          const errorMsg = 'Proof of payment file is invalid. Please upload again.'
          setError(errorMsg)
          onError(errorMsg)
          setLoading(false)
          return
        ...
        // Recreate File object from ArrayBuffer (prevents JSON serialization)
        ...
        console.log('📤 Submitting mobile banking payment with proof:', {
          fileName: file.name,
          fileType: file.type,
          fileSize: file.size
        })
        ...
        // Keep loading state active - parent will handle completion
        ...
        // Don't reset loading here - let parent handle it via processingPayment prop
        return
      }
      ...
      if (confirmError) {
        ...
        if (paymentIntent.status === 'succeeded') {
          // Payment succeeded - keep loading state active for parent to handle
          paymentSucceeded = true
      ...
      // Only reset loading if payment didn't succeed (parent will handle success state)
      if (!paymentSucceeded) {
        setLoading(false)
      }
    }
            ...
            <>
              ...
              <div>
                        ...
                        <div className="flex items-center gap-2 flex-1 min-w-0">
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <p className="text-xs text-gray-500 dark:text-gray-400">
                              {(proofOfPaymentData.fileSize / 1024 / 1024).toFixed(2)} MB
                            </p>
                          </div>
                        </div>
                        <button
                          ...
                          onClick={() => {
                            ...
                            if (proofOfPaymentPreview) {
                              URL.revokeObjectURL(proofOfPaymentPreview)
                            }
                            setProofOfPaymentPreview(null)
                            setUploadError(null)
                    ...
                    <div
                      ...
                      onDragOver={(e) => {
                        ...
                        if (dropZoneRef.current) {
                          dropZoneRef.current.classList.add('border-primary-500', 'bg-primary-50', 'dark:bg-primary-900/20')
                        }
                      }}
                      onDragLeave={(e) => {
                        e.preventDefault()
                        e.stopPropagation()
                        if (dropZoneRef.current) {
                          dropZoneRef.current.classList.remove('border-primary-500', 'bg-primary-50', 'dark:bg-primary-900/20')
                        }
                      }}
                      onDrop={(e) => {
                        e.preventDefault()
                        ...
                        <div className="flex flex-col items-center gap-2">
                          ...
                          <div>
                            ...
                            <span className="text-xs text-gray-500 dark:text-gray-400 mt-1 block">
                              Screenshot or receipt (JPG, PNG, WebP, or PDF)
                            </span>
                          </div>
                        </div>
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">
                  📸 Upload a clear screenshot or receipt showing your payment transaction. Files are automatically compressed to ensure fast upload and prevent corruption.
                </p>
              </div>
            </>
          )}
      ...
      >
        {isProcessing ? (
          <>
            ...
            {externalProcessing ? 'Completing Payment...' : 'Processing Payment...'}
          </>
        ) : (
          <>
